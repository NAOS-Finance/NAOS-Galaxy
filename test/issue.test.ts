...
describe("Issue", function () {
  ...
  const issueLoan = async (tokenId: BigNumber, lookupId: string) => {
    const loanId = await borrower.callStatic.issue(collateralNFT.address, tokenId)
    await borrower.issue(collateralNFT.address, tokenId)
    await assertPostCondition(loanId, tokenId, lookupId)
  }
  ...
  const assertPostCondition = async (
    ...
    expect(await shelf.nftlookup(lookupId)).to.be.eq(loanId)
    expect(await title.ownerOf(loanId)).to.be.eq(borrower.address)
    expect(await collateralNFT.ownerOf(tokenId)).to.be.eq(borrower.address)
  }

  it("Should IssueLoan", async () => {
    const { tokenId, lookupId } = await issueNFT(borrower.address)
    await assertPreCondition(tokenId, lookupId)
    await issueLoan(tokenId, lookupId)
  })

  it("Should FailIssueMultipleLoansForOneNFT", async () => {
    const { tokenId, lookupId } = await issueNFT(borrower.address)
    await issueLoan(tokenId, lookupId)
    expect(
      borrower.callStatic.issue(collateralNFT.address, tokenId)
    ).to.be.revertedWith("")
    // const secondLoanId = await borrower.callStatic.issue(collateralNFT.address, tokenId)
    // await borrower.issue(collateralNFT.address, tokenId)
    // await assertPostCondition(secondLoanId, tokenId, lookupId)
  })

  it("Should FailIssueLoanNotNFTOwner", async () => {
