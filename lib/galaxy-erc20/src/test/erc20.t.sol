...
contract ERC20User {
    ...
    function doApprove(address usr) public returns (bool) {
        return erc20.approve(usr, uint256(-1));
    }
    ...
    function doMint(address usr, uint256 wad) public {
        erc20.mint(usr, wad);
    }

    function doBurn(address usr, uint256 wad) public {
        erc20.burn(usr, wad);
    }
}
...
contract ERC20Test is DSTest {
    ...
    function testTrusting() public {
        assertEq(erc20.allowance(self, user2), 0);
        erc20.approve(user2, uint256(-1));
        assertEq(erc20.allowance(self, user2), uint256(-1));
        erc20.approve(user2, 0);
        assertEq(erc20.allowance(self, user2), 0);
    }

    function testTrustedTransferFrom() public {
        erc20.approve(user1, uint256(-1));
        ERC20User(user1).doTransferFrom(self, user2, 200);
    ...
    function testApproveWillNotModifyAllowance() public {
        ...
        assertEq(erc20.balanceOf(user1), 0);
        erc20.approve(user1, uint256(-1));
        assertEq(erc20.allowance(self, user1), uint256(-1));
    ...
    function testPermit() public {
        ...
        erc20.permit(cal, del, 10000, uint256(-1), v, r, s);
    ...
    function testFailReplay() public {
        erc20.permit(cal, del, 0, uint256(-1), v, r, s);
        erc20.permit(cal, del, 0, uint256(-1), v, r, s);
    }
}
