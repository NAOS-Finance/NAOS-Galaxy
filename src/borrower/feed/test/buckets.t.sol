...
contract BucketList is Buckets, Math {
    ...
    function add(uint256 timestamp, uint256 value) public {
        addBucket(uniqueDayTimestamp(timestamp), value);
    }

    function remove(uint256 timestamp) public {
        removeBucket(uniqueDayTimestamp(timestamp));
    }

    function calcSum() public view returns (uint256) {
        uint256 currDate = firstBucket;
        uint256 sum = 0;
