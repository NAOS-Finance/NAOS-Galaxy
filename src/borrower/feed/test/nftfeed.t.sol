...
contract NFTFeedTest is DSTest, Math {
    ...
    function init() public {
        ...
        nftFeed.file("riskGroupNFT", 1, 7 * 10**26, 5 * 10**26, uint256(1000000003593629043335673583));

        // risk group  => 2
        // thresholdRatio => 70%
        // ceilingRatio => 50%
        // interestRate => 5 % per day
        nftFeed.file("riskGroupNFT", 2, 7 * 10**26, 5 * 10**26, uint256(1000000564701133626865910626));

        // risk group  => 3
        // ceiling ratio => 100%
        // thresholdRatio => 70%
        // interest rate => 5% per day
        nftFeed.file("riskGroupNFT", 3, 7 * 10**26, ONE, uint256(1000000564701133626865910626));

        // risk group => 4 (used by collector tests)
        // ceiling ratio => 50%
        // thresholdRatio => 60%
        // interest rate => 5% per day
        nftFeed.file("riskGroupNFT", 4, 6 * 10**26, 5 * 10**26, uint256(1000000564701133626865910626));
    }
    ...
    function testUpdateNFTCeilingExceedsBorrowedAmount() public {
        ...
        uint256 maxCeiling = nftFeed.ceiling(loan);
        assertEq(nftFeed.ceiling(loan), maxCeiling);
