...
contract CoordinatorExecuteEpochTest is CoordinatorTest {
    ...
    function prepareExecute(LenderModel memory model_, ModelInput memory input) public {
        ...
        int256 result = coordinator.submitSolution(input.seniorRedeem, input.juniorRedeem, input.juniorSupply, input.seniorSupply);
        // new best solution
    ...
    function checkTrancheUpdates(LenderModel memory model_, ModelInput memory input) public {
        assertEq(seniorTranche.values_uint("epochUpdate_supplyFulfillment"), rdiv(input.seniorSupply, model_.seniorSupplyOrder));

        assertEq(seniorTranche.values_uint("epochUpdate_redeemFulfillment"), rdiv(input.seniorRedeem, model_.seniorRedeemOrder));

        assertEq(juniorTranche.values_uint("epochUpdate_supplyFulfillment"), rdiv(input.juniorSupply, model_.juniorSupplyOrder));

        assertEq(juniorTranche.values_uint("epochUpdate_redeemFulfillment"), rdiv(input.juniorRedeem, model_.juniorRedeemOrder));
    }
    ...
    function testExecuteEpoch() public {
        ...
        ModelInput memory input = ModelInput({seniorSupply: 1 ether, juniorSupply: 2 ether, seniorRedeem: 3 ether, juniorRedeem: 4 ether});
        ...
        uint256 shouldRatio = rdiv(seniorAsset, safeAdd(shouldNewReserve, model_.NAV));

        uint256 currSeniorRatio = coordinator.calcSeniorRatio(shouldSeniorAsset, model_.NAV, shouldNewReserve);
    ...
    function testCalcSeniorState() public {
        ...
        uint256 currSeniorAsset = 0;
        uint256 seniorRedeem = 0;
        uint256 seniorSupply = 0;

        uint256 seniorAsset = coordinator.calcSeniorAssetValue(seniorRedeem, seniorSupply, currSeniorAsset, model.reserve, model.NAV);
        ...
        uint256 newReserve = coordinator.calcNewReserve(seniorRedeem, 0, seniorSupply, 0);
