...
contract CoordinatorSubmitEpochTest is CoordinatorTest, FixedPoint {
    ...
    function testMaxImprovementScore() public {
        ...
        assertTrue(score <= uint256(-1));

        uint256 maxDistancePoints = rmul(coordinator.IMPROVEMENT_WEIGHT(), rdiv(ONE, 1));
    ...
    function testSubmitSolution() public {
        ...
        ModelInput memory solution = ModelInput({seniorSupply: 1 ether, juniorSupply: 2 ether, seniorRedeem: 3 ether, juniorRedeem: 4 ether});

        assertEq(submitSolution(solution), coordinator.SUCCESS());
        compareWithBest(solution);

        // challenge period started
        uint256 challengeEnd = now + 1 hours;
        assertEq(coordinator.minChallengePeriodEnd(), challengeEnd);

        hevm.warp(now + 2 hours);

        ModelInput memory betterSolution = ModelInput({seniorSupply: 2 ether, juniorSupply: 3 ether, seniorRedeem: 4 ether, juniorRedeem: 5 ether});
    ...
    function checkPoolPrecondition(
        LenderModel memory model,
        bool currSeniorRatioInRange,
        bool reserveHealthy
    ) public {
    ...
    function testSubmitEpochUnhealthyState() public {
        ...
        ModelInput memory solution = ModelInput({seniorSupply: 0 ether, juniorSupply: 0 ether, seniorRedeem: 100 ether, juniorRedeem: 100 ether});
    ...
    function testSubmitImprovement() public {
        ...
        ModelInput memory solution = ModelInput({seniorRedeem: 0 ether, juniorSupply: 0 ether, seniorSupply: 800 ether, juniorRedeem: 0 ether});

        assertEq(submitSolution(solution), coordinator.SUCCESS());
        assertTrue(coordinator.gotFullValidSolution() == false);

        solution = ModelInput({seniorRedeem: 0 ether, juniorSupply: 0 ether, seniorSupply: 800 ether, juniorRedeem: 500 ether});

        assertEq(submitSolution(solution), coordinator.SUCCESS());
        assertTrue(coordinator.gotFullValidSolution() == false);

        solution = ModelInput({seniorRedeem: 0 ether, juniorSupply: 0 ether, seniorSupply: 300 ether, juniorRedeem: 1000 ether});

        assertEq(submitSolution(solution), coordinator.SUCCESS());
        assertTrue(coordinator.gotFullValidSolution() == false);

        // solution would satisfy all constraints
        solution = ModelInput({seniorRedeem: 0 ether, juniorSupply: 0 ether, seniorSupply: 0 ether, juniorRedeem: 950 ether});
        ...
        // solution would satisfy all constraints
        solution = ModelInput({seniorRedeem: 50 ether, juniorSupply: 0 ether, seniorSupply: 250 ether, juniorRedeem: 950 ether});
    ...
    function submitSolutionWorseThanBenchmark() public {
        ...
        ModelInput memory solution = ModelInput({seniorRedeem: 0 ether, juniorSupply: 100 ether, seniorSupply: 0 ether, juniorRedeem: 0 ether});
    ...
    function testNoImprovementPossibleReserveViolated() public {
        ...
        ModelInput memory solution = ModelInput({seniorRedeem: 0 ether, juniorSupply: 0 ether, seniorSupply: 0 ether, juniorRedeem: 0 ether});
        ...
        solution = ModelInput({seniorRedeem: 0 ether, juniorSupply: 0 ether, seniorSupply: 1 ether, juniorRedeem: 0 ether});
    ...
    function testNoImprovementPossibleRatioViolated() public {
        ...
        solution = ModelInput({seniorRedeem: 0 ether, juniorSupply: 1 ether, seniorSupply: 0 ether, juniorRedeem: 0 ether});
